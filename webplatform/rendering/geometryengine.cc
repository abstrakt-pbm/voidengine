...
GeometryEngine::CalculateNodeGeometry(const DomNode &dom_node,
  ...
  } else if (auto *element_node =
                 dynamic_cast<const ElementNode *>(&dom_node)) {
    return CalculateElementNodeGeometry(*element_node, constrains);
  }
...
std::unique_ptr<PhysicalFragment> GeometryEngine::CalculateDocumentGeometry(
  ...
  return CalculateNodeGeometry(nonconst_htmlelem, constrains);
}

std::unique_ptr<PhysicalFragment> GeometryEngine::CalculateElementNodeGeometry(
  ...
  for (const auto &child_element : container_node.childs_) {
    auto child_fragment = CalculateNodeGeometry(*child_element, new_constrains);
